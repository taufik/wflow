...
class DashboardView extends Views
{
	...
	{
    ?>
    <div id='MainLayout' class='mainLayout' style="min-height:1024px;"></div>
    <script>
...
<?php
		...
		$rows=array();
			...
			foreach($events as $event)
			{
				$row=array();
					...
					{
						$link=Helper::getUrl(array('method'=>$action,'processId'=>$event['processId'])); 
	...
	//		$pid=$row['processNodeId'];
			...
			$linkCase=Helper::getUrl(array('class'=>'CaseController','method'=>'Action_view','caseId'=>$cid));

			//$link=Helper::getUrl(array('action'=>'task.execute','caseId'=>$cid,'id'=>$id));
			$link=Helper::getUrl(array('class'=>'TaskController','method'=>'Action_execute','caseId'=>$cid,'id'=>$id));
