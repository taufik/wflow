...
class Form {
    ...
    //public function getHtml($item, $edit ) {
        ...
        foreach ($task->dataElements as $var) {
			...
			$edit =  ( null !== $var->canEdit() )? $var->canEdit():"";
            $view = ( null  !== $var->canView() )? $var->canView():"";
			$var = isset($var) ? $var : '';
		
            Context::Log("INFO", "default form field name: $name type: " . print_r($var, true) . 'edit:' . $edit . 'view:' . $view);
            if ((!$view) && (!$edit))
                continue;
    ...
    private function header($title, $caseId, $id) {
        $h1 = Helper::getUrl(array(
			"class" => "TaskController",	
            "method" => "Action_saveForm",
            "case" => $caseId,
            "item" => $id
        ));
...
?>	
   ...
   <h2 class='formTitle' style='margin-bottom: 15px;'><?php
        echo $title;
?></h2>
   <div >
   <form class="form-horizontal" type="bootstrap" enctype="multipart/form-data" name="dataform" id="dataform" method="post">

	 <input type='hidden' name='_caseId' value='<?php
        echo $caseId;
?>' />
	 <input type='hidden' name='_itemId' value='<?php
        echo $id;
?>' />
<?php
...
?>
 <button id="tbutton_btn_save" name="btn_save" class="btn btn-sm btn-primary" onclick="Adianti.waitMessage = 'Loading';__adianti_post_data('dataform', 'class=TaskController&method=Action_saveForm');return false;"><span>
		<i style="padding-right:4px;;padding-right:4px" class="fa fa-save"></i>
		Save</span>
</button>
<?php		
        //Field::OtherField("save", "", "<input class='btn btn-primary' type='submit'  value='Save' />", "");
        echo "</form>";
        if ($releaseStatement) {
				echo "
