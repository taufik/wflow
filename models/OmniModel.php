...
class OmniModel {
    ...
    public function listEvents() {
        ...
        $sql     = "select 'Case Item' as source,id,null as processName, processNodeId,caseId,type,subType,label,timer,timerDue,message,signalName " . " from $table " . " where  $status" . " and subType in('timer','message','signal')";
        $arr1    = $this->db->select($sql);
        $sql     = "select 'Process Item' as source ,p.processName as processName, pi.id as id,pi.processNodeId,null as caseId,pi.type,subType,label,timer,timerDue,message,signalName " . " from $piTable pi
    ...
    public function getMessageHandler($message) {
        ...
        $piTable = ProcessItemModel::getInstance()->getTable();
        $status  = "status not in ('Complete','Terminated') ";
        $sql     = "select 'Case Item' as source,id,null as processId, processNodeId,caseId,type,subType,label,timer,timerDue,message,signalName " . " from $table " . " where  $status" . " and message='$message'";
        $arr1    = $this->db->select($sql);
        $sql     = "select 'Process Item' as source ,p.processId as processId, pi.id as id,pi.processNodeId,null as caseId,pi.type,subType,label,timer,timerDue,message,signalName " . " from $piTable pi
                            join $pTable  p on p.processId=pi.processId
    ...
    public function getSignalHandler($message) {
        ...
        $piTable = ProcessItemModel::getInstance()->getTable();
        $status  = "status not in ('Complete','Terminated') ";
        $sql     = "select 'Case Item' as source,id,null as processId, processNodeId,caseId,type,subType,label,timer,timerDue,message,signalName " . " from $table " . " where  $status" . " and signalName='$message'";
        $arr1    = $this->db->select($sql);
        $sql     = "select 'Process Item' as source ,p.processId as processId, pi.id as id,pi.processNodeId,null as caseId,pi.type,subType,label,timer,timerDue,message,signalName " . " from $piTable pi
                            join $pTable  p on p.processId=pi.processId
    ...
    public function listRecents($forAllUsers = false) {
        ...
        $tblU   = $this->db->getPrefix() . "system_user";
        ...
        $sql   = "SELECT u.name as userName , a.userGroup , ci.label,ci.caseId,ci.id, c.processName,
    ...
    public function listTasks($forAllUsers = false) {
        ...
        $sql = "  SELECT u.name as userName , a.userGroup , ci.label,a.caseId,ci.id, 
